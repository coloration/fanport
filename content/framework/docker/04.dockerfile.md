...
NOTE: 如果你基于某个镜像构建自己的镜像，这个镜像定义了自己的 ENTRYPOINT 则你需要使用自己的 ENTRYPOINT 对其进行覆盖 e.g.
...
# Override the ENTRYPOINT to execute your script
ENTRYPOINT ["/start.sh"]
...
# 这里注意复制整个目录文件时，需要在 .dockerignore 添加 node_modules 防止覆盖
COPY . .
